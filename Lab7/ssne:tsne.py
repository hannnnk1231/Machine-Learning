...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    for iter in range(max_iter):
        ...
        #plt.savefig('png/tsne/tsne-iter{}.png'.format(iter+1))
        #plt.show()
...
def ssne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    for iter in range(max_iter):
        ...
        #plt.savefig('png/ssne/ssne-iter{}.png'.format(iter+1))
        #plt.show()
    # Return solution
    return Y,P,Q
...
if __name__ == "__main__":
	...
	plt.savefig('png/tSNE High-D similarity')
	plt.title('tSNE,Low-D similarity')
	plt.imshow(Q1,cmap='hot')
	plt.savefig('png/tSNE Low-D similarity')
	plt.title('SSNE,High-D similarity')
	plt.imshow(P2,cmap='hot')
	plt.savefig('png/SSNE High-D similarity')
	plt.title('SSNE,Low-D similarity')
	plt.imshow(Q2,cmap='hot')
	plt.savefig('png/SSNE Low-D similarity')

	for i in [3,5,20,50,100]:
    	Y3,P3,Q3 = ssne(X,2,50,i)
    	pylab.scatter(Y3[:, 0], Y3[:, 1], 10, labels)
    	plt.title('s-SNE, perplexity {}'.format(i))
    	plt.savefig('png/ssne-per{}.png'.format(i))
    	plt.show()
    	Y3,P3,Q3 = tsne(X,2,50,i)
    	pylab.scatter(Y3[:, 0], Y3[:, 1], 10, labels)
    	plt.title('t-SNE, perplexity {}'.format(i))
    	plt.savefig('png/tsne-per{}.png'.format(i))
    	plt.show()